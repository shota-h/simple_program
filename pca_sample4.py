...
def Load_test():
    for i in range(0,class_num):
        for  j in range(0,test_num):
            ...
            img=cv2.imread('programe/pattern_recognition/Images/TestSamples/'+img_name,0)
            if img is None:
                print(img_name)
                print('not read')
                sys.exit()
...
def Load_train():
    for i in range(0,class_num):
        for  j in range(0,train_num):
            jj='{0:04d}'.format(j)
            img_name=str(i)+'-'+str(jj)+file_ex
            train_img=cv2.imread('programe/pattern_recognition/Images/TrainingSamples/'+img_name,0)
...
def knn(train_pca,train_class):
    ...
    for i,j in itertools.product(range(class_num),range(test_num)):
        ...
        norm_train=np.linalg.norm(train_pca.T-test_row[:,i*test_num+j].T,axis=1)
        min_n=np.argmin(norm_train)
        # print(min_n)
        if test_class[i*test_num+j]==train_class[min_n]:
            score_all[i]+=1
    print('score',score_all/1000)
    print('ave',np.mean(score_all)/1000)
...
if __name__=='__main__':
    ...
    #     mean_p=np.mean(img_pca[n:(n+1)*img_num,:],axis=0)
